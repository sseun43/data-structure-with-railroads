...
std::vector<StationID> Datastructures::stations_closest_to(Coord xy)
{
    ...
    if(vector_of_station_coord.size() == multimap_of_station_coord.size()){
        ...
        int partial_k = 3;
        if (vectorOfId.size() < 3) {
            partial_k = vectorOfId.size() -1;
        }
        std::partial_sort(vectorOfId.begin(), vectorOfId.begin() + partial_k, vectorOfId.end(), comp);
...
{
    // Replace the line below with your implementation
    // save to map of trainid : vectorStationID
    auto trainSearch = map_of_train_vectorOfStations.find(trainid);
...
{
    // Replace the line below with your implementation
    if (get_station_coordinates(id) == NO_COORD) {
        // not found
        return {NO_STATION};
    } else {
...
std::vector<StationID> Datastructures::train_stations_from(StationID stationid, TrainID trainid)
{
    // Replace the line below with your implementation 
    auto trainSearch = map_of_train_vectorOfStations.find(trainid);
    if (trainSearch == map_of_train_vectorOfStations.end() || get_station_coordinates(stationid) == NO_COORD) {
        // not found
        return {NO_STATION};
    } else {
    ...
    // if found find the station position in the train vector and return all the station id till end
